	...
	{ -- Screen
		...
		render = function(self, x, y, opac)
			...
			elseif status == self.Status_Granted then
				surface.SetFont("KeypadStatus")
				surface.SetTextColor(Color(0, 255, 0, opac))
...
function ENT:Paint(w, h, x, y)
	if not self:KPShouldDraw() then return end
	if math.Round(self.Opac) == 0 then return end

	local hovered = self:GetHoveredElement()

	for k, element in ipairs(elements) do
		local col = Color(element.color.r, element.color.g, element.color.b, self.Opac)
		...
		if element == hovered and element.hovercolor then
			local colr = Color(element.hovercolor.r, element.hovercolor.g, element.hovercolor.b, self.Opac)
		...
		if element.text then
			...
			surface.SetTextColor(Color(0, 0, 0, self.Opac))
